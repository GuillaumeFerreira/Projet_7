...
class Optimized:
    ...
    def run(cls, store, route_params):
        ...
        store.get_action()
        poid,benef,elements = cls.sacADos_dynamique(store.data['invest'], store.data['actions'])
        print(str(elements) +' benefice = '+ str(benef) + ' pour un investissement de '+str(poid))
        ...
        for i in range(1, len(elements) + 1):
            for w in range(1, capacite + 1):
                if elements[i-1].cpa <= w:
                    matrice[i][w] = max(elements[i-1].benef + matrice[i-1][w-elements[i-1].cpa], matrice[i-1][w])
                else:
