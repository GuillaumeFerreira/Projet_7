...
class Optimized:
    ...
    def sacADos_dynamique(cls, store, route_params):
        ...
        # On rempli la matrice , on va regarder pour chaque cout
        # la valeur la plus rentable
        """4 valeurs [ [poid = 1 ,val = 1], [poid = 3 ,val = 4] ,
        [poid = 4 ,val = 5] ,[poid = 5 ,val = 7] ]-->
        'shares_total' et un investissement de 7--> 'max_inv'

        Si le le cout est suffisant on fait max( valeur de l element regardé
        + valeur de la case de la ligne d avant et pour la colonne on se place
        en fonction du cout utilisé ,ou on prend la meme colonne de la ligne
        precedente) cost[1, 3, 4, 5] , profit[1, 4, 5, 7]
